...
void solve() {
    ...
    vi ss, tt; fin(it, s) ss.pb(it - '0'); fin(it, t) tt.pb(it - '0');

    auto st = math::polymul(ss, tt);
    vll res; ll carry = 0;
    fin(it, st) {
        ll cur = it.real() + 0.5;
        cur += carry;
    ...
    res.pb(carry);
    while (sz(res) > 1 && res.back() == 0) res.ppb();
    reverse(all(res)); if (neg && res[0]) cout<<"-";fin(it,res) cout<<it; cout << endl;
}

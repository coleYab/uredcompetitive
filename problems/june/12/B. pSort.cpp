...
void solve() {
    int n; cin >> n;
    vi a(n); fin(it, a) cin >> it, it--;
    vi b(n); fin(it, b) cin >> it;

    dsu_t dsu(n);
    fr(i, 0, n) {
        int k = b[i];
        if (i >= k) dsu.merge(i - k, i);
        if (i + k < n) dsu.merge(i + k, i);
    }

    map<int, vi> cords;
    map<int, vi> cord2;
    fr(i, 0, n) cords[dsu.find(i)].pb(a[i]);
    fr(i, 0, n) cord2[dsu.find(i)].pb(i);
    fr(i, 0, n + 1) {
        vi a = cords[i], b = cord2[i];
